...
```bash
npm install -g wechat-export-macos
```

安装后即可全局使用 `wechat-export` 命令。
...
```bash
# 列出所有会话（按消息数排序）
wechat-export export -l

# 导出某个联系人的聊天记录（模糊搜索昵称/备注）
wechat-export export -n "张三"

# 指定输出目录
wechat-export export -n "张三" -o ~/Downloads/mychat

# 直接指定用户名导出
wechat-export export -u wxid_xxxxx
```
...
| 命令 | 说明 |
|------|------|
| `wechat-export export [options] [name]` | 导出聊天记录 |
| `wechat-export decrypt` | 解密微信数据库 |
| `wechat-export keys` | 从微信进程提取密钥（需 sudo） |
| `wechat-export doctor` | 检测环境依赖 |
